...
def run_rl_episode(config, rng, policy, device, deterministic=True):
...
def run_rl_batch(config, num_runs, policy, device, deterministic=True):
    ...
    for run_index in range(num_runs):
        ...
        outcome, steps, _reward = run_rl_episode(
            run_config, rng, policy, device, deterministic=deterministic,
        )
        accumulator.runs += 1
...
def evaluate_policy(
    ...
    for num_prey in prey_counts:
        ...
        summary = run_rl_batch(
            config, num_runs, policy, device, deterministic=True,
        )
